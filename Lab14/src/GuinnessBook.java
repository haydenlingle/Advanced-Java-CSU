...
public class GuinnessBook {
	...
	public GuinnessBook(String filename) throws FileNotFoundException{
		...
		while(scan.hasNextLine()){
			...
			Animal newAnimal = new Animal(animalName, topSpeed);
			landAnimals.add(newAnimal);//insert the animal into the arraylist here, don't change any of the above code
	...
	public String toString(){
		...
		//return "hi";// ("Name: " + newAnimal.getName() + " Top Speed: " + this.topSpeed);
	}
	...
	private void testGuinnessBook() throws FileNotFoundException {
        ...
        for(int i = 0; i < landAnimals.size(); i++){
        	if(landAnimals.get(i).getTopSpeed() >= 48 && landAnimals.get(i).getTopSpeed() <= 52){
